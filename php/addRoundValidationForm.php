<?php
	...
	if ($result->num_rows == 0)
	{
		if ($rels == "rels")
		{
            ...
            if (isset($eventId) && isset($leaderId)) {
                $query = "INSERT INTO rounds (round_no, event_id, leader_id, state) VALUES ('$RoundNo', '$eventId', '$leaderId', '$state')";
                $res = $conn->query($query);
            }
            else {
                $_SESSION["Msg"] = "Invalid event id and leader id";
                header("Location: rounds.php");
            }
	...
	else if ($result->num_rows > 0)
	{
		...
		if ($RoundNo == $data["round_no"] && $eventId == $data["event_id"])
		{
			$_SESSION["RoundNoError"] = "Round number " . $RoundNo . " Already exist in " . $eventId;
			header("Location: rounds.php");
		}
		else
		{
